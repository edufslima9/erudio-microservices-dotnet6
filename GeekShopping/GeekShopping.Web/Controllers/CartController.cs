...
namespace GeekShopping.Web.Controllers
{
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> ApplyCoupon(CartViewModel model)
        {
            ...
            var response = await _cartService.ApplyCoupom(model, token);
            if (response)
            {
        ...
        public async Task<IActionResult> RemoveCoupon()
        {
            ...
            var response = await _cartService.RemoveCoupom(userId, token);
            if (response)
            {
                return RedirectToAction(nameof(CartIndex));
            }
        ...
        private async Task<CartViewModel> FindUserCart()
        {
            ...
            if (response?.CartHeader != null)
            {
                ...
                {
                    var coupon = await _couponService.GetCouponByCouponCode(response.CartHeader.CouponCode, token);
